function [fig_handle, axes_handle, scroll_bar_handles, scroll_func] = ...
	...
	key_func, varargin)
%VIDEOFIG Figure with horizontal scrollbar and play capabilities.          VIDEOFIG具有水平滚动条和播放功能的图形。
%   VIDEOFIG(NUM_FRAMES, @REDRAW_FUNC)
%   Creates a figure with a horizontal scrollbar and shortcuts to scroll   使用水平滚动条和快捷方式自动滚动创建图形。
%   automatically. The scroll range is 1 to NUM_FRAMES. The function       滚动范围是1到NUM_FRAMES。
%   REDRAW_FUNC(F) is called to redraw at scroll position F (for example,  调用函数REDRAW_FUNC（F）在滚动位置F重绘
%   REDRAW_FUNC can show the frame F of a video).                         （例如，REDRAW_FUNC可以显示视频的帧F）。
%   This can be used not only to play and analyze standard videos, but it  这不仅可用于播放和分析标准视频，
%   also lets you place any custom Matlab plots and graphics on top.       还可让您将任何自定义的Matlab绘图和图形置于顶层。
%
%   The keyboard shortcuts are:                                            键盘快捷键是：
%     Enter (Return) -- play/pause video (25 frames-per-second default).   Enter (Return)- 播放/暂停视频（默认为25帧/秒）。
%     Backspace -- play/pause video 5 times slower.                        Backspace  - 播放/暂停视频慢5倍。
%     Right/left arrow keys -- advance/go back one frame.                  向右/向左箭头键 - 前进/返回一帧。
%     Page down/page up -- advance/go back 30 frames.                      Page down/page up - 向前翻页/向后翻页30帧。
%     Home/end -- go to first/last frame of video.                         Home/end - 转到视频的第一帧/最后一帧。
%
%   Advanced usage                                                         高级用法
%   --------------
%   VIDEOFIG(NUM_FRAMES, @REDRAW_FUNC, FPS, BIG_SCROLL)
%   Also specifies the speed of the play function (frames-per-second) and  还指定播放功能的速度（帧每秒）和
%   the frame step of page up/page down (or empty for defaults).           页面向上/向下翻页（或默认为空）的帧步骤。
%
%   VIDEOFIG(NUM_FRAMES, @REDRAW_FUNC, FPS, BIG_SCROLL, @KEY_FUNC)
%   Also calls KEY_FUNC(KEY) with any keys that weren't processed, so you  还会调用KEY_FUNC（KEY）与任何未处理的键，
%   can add more shortcut keys (or empty for none).                        因此您可以添加更多快捷键（或者为空）。
%
%   VIDEOFIG(NUM_FRAMES, @REDRAW_FUNC, FPS, BIG_SCROLL, @KEY_FUNC, ...)
%   Passes any additional arguments to the native FIGURE function (for     将任何附加参数传递给本地图形函数
%   example: 'Name', 'Video figure title').                               （例如：'Name'，'Video figure title'）。
%
%   [FIG_HANDLE, AX_HANDLE, OTHER_HANDLES, SCROLL] = VIDEOFIG(...)
%   Returns the handles of the figure, drawing axes and other handles (of  分别返回图形的手柄，绘图轴和其他手柄（滚动条的图形）。
%   the scrollbar's graphics), respectively. SCROLL(F) can be called to
%   scroll to frame F, or with no arguments to just redraw the figure.     可以调用SCROLL（F）来滚动到F帧，或者不带任何参数来重绘图形。
%
%   Example 1
%   ---------
...
%   Jo鉶 Filipe Henriques, 2010
	...
	if nargin < 3 || isempty(play_fps), play_fps = 25; end                 %||：或
    %play speed (frames per second)                                        播放速度（每秒帧数）
	if nargin < 4 || isempty(big_scroll), big_scroll = 30; end 
    %page-up and page-down advance, in frames                              以帧为单位向上翻页和向下翻页
	if nargin < 5, key_func = []; end
	
	%check arguments                                                       检查参数
	check_int_scalar(num_frames);                                          %在最后定义了这两个函数
	...
	%scroll bar                                                            滚动条                                                           
	scroll_bar_width = max(1 / num_frames, 0.01);                          %max：返回一个和A和B同大小的数组，其中的元素是从A或B中取出的最大元素
	...
	%patch：用来创建补丁图形对象。一个补丁对象是由其顶点坐标确定的一个或多个多边形。用户可以指定补丁对象的颜色等信息。
	%timer to play vide                                                    播放视频的定时器
	play_timer = timer('TimerFcn',@play_timer_callback, 'ExecutionMode','fixedRate');%'ExecutionMode',定时器的执行模式。
	%'TimerFcn'：定时器的回调函数（自定义），'fixedRate'：第一次TimeFcn开始排队到第二次TimeFcn开始排队之间的时间为定时周期
	%main drawing axes for video display                                   视频显示的主要绘图轴
